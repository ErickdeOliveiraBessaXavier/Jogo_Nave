...
def get_function_stats(stats: pstats.Stats) -> Dict[str, Dict]:
    ...
    # Acessar dados internos do pstats
...
def compare_profiles(before_stats: pstats.Stats, after_stats: pstats.Stats) -> Dict:
    """Compara dois perfis e calcula diferenças."""
    before_data = get_function_stats(before_stats)
    after_data = get_function_stats(after_stats)
    ...
    before_total = sum(func['tottime'] for func in before_data.values())
    after_total = sum(func['tottime'] for func in after_data.values())

    comparison['summary'] = {
        'before_total_time': before_total,
        'after_total_time': after_total,
        'time_difference': after_total - before_total,
        'time_improvement_percent': ((before_total - after_total) / before_total * 100) if before_total > 0 else 0,
        'total_calls_before': sum(func['ncalls'] for func in before_data.values()),
        'total_calls_after': sum(func['ncalls'] for func in after_data.values())
    }
    ...
    for func_name in common_functions:
        ...
        time_diff = after['tottime'] - before['tottime']
        time_percent = (time_diff / before['tottime'] * 100) if before['tottime'] > 0 else 0

        calls_diff = after['ncalls'] - before['ncalls']

        result = {
            'function': func_name,
            'before_time': before['tottime'],
            'after_time': after['tottime'],
            'time_difference': time_diff,
            'time_percent_change': time_percent,
            'before_calls': before['ncalls'],
            'after_calls': after['ncalls'],
    ...
    for func_name in set(after_data.keys()) - common_functions:
        comparison['new_functions'].append({
            'function': func_name,
            'time': after_data[func_name]['tottime'],
            'calls': after_data[func_name]['ncalls']
        })
    ...
    for func_name in set(before_data.keys()) - common_functions:
        comparison['removed_functions'].append({
            'function': func_name,
            'time': before_data[func_name]['tottime'],
            'calls': before_data[func_name]['ncalls']
        })

    # Ordenar por impacto
    comparison['improved'].sort(key=lambda x: abs(x['time_percent_change']), reverse=True)
    comparison['worsened'].sort(key=lambda x: abs(x['time_percent_change']), reverse=True)
...
""")
    ...
    if summary['time_improvement_percent'] > 0:
        ...
        print(f"🔴 Performance piorou em {abs(summary['time_improvement_percent']):.1f}%")
    else:
    ...
    if comparison['improved']:
        ...
        for func in comparison['improved'][:10]:  # Top 10
            ...
            print(f"   Tempo: {func['before_time']:.3f}s → {func['after_time']:.3f}s ({func['time_difference']:+.3f}s)")
    ...
    if comparison['new_functions']:
        print("\n🆕 FUNÇÕES NOVAS:")
        print("-" * 80)
        for func in sorted(comparison['new_functions'], key=lambda x: x['time'], reverse=True)[:5]:
            print(f"➕ {func['function']}: {func['time']:.3f}s ({func['calls']:.0f} chamadas)")
            print()
    ...
    if comparison['removed_functions']:
        print("\n🗑️  FUNÇÕES REMOVIDAS:")
        print("-" * 80)
        for func in sorted(comparison['removed_functions'], key=lambda x: x['time'], reverse=True)[:5]:
            print(f"➖ {func['function']}: {func['time']:.3f}s ({func['calls']:.0f} chamadas)")
            print()
    ...
    if summary['time_improvement_percent'] > 5:
        ...
        print("🤔 Performance estável. Verificar se otimizações foram aplicadas corretamente.")

    # Análise específica do StarField
    starfield_improved = any('StarField.draw' in func['function'] for func in comparison['improved'])
...
def main():
    if len(sys.argv) != 3:
        print("Uso: python comparador_perfis.py <perfil_antes.prof> <perfil_depois.prof>")
        print("\nExemplo:")
