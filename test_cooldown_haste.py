...
class MockEntityManager:
    player_lasers = []
    def spawn_player_laser(self, *args, **kwargs): pass

class MockScene:
    screen_shake_timer = 0

class MockSoundManager:
    def play_laser(self): pass
    def play_upgrade_activate(self): pass
    def play_upgrade_denied(self): pass
...
assert laser.cooldown_left == 40.0, f"LASER_SHOT deveria ter 40s, tem {laser.cooldown_left}s"
assert shield.cooldown_left == 25.0, f"SHIELD_BURST deveria ter 25s, tem {shield.cooldown_left}s"
...
laser.cooldown_left = max(0.0, laser.cooldown_left - REDUCTION)
print(f"    Após redução de {REDUCTION}s: {laser.cooldown_left:.1f}s (esperado: 0s, não negativo)")

assert laser.cooldown_left == 0.0, f"Cooldown não pode ser negativo! Valor: {laser.cooldown_left}"
