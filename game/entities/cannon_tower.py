...
class CannonTower:
    ...
    def can_fire_next_charge(self) -> bool:
        if self.all_charges_used: return False
        if self.mines_fired_in_current_charge >= self.mines_per_charge:
            active_mines_count = len([mine for mine in self.active_mines if not mine.dead])
    ...
    def update(self, dt: float) -> None:
        ...
        if self.blinking:
            self.blink_timer += dt
            self.blink_visible = (int(self.blink_timer * 10) % 2) == 0 # Piscar mais rápido
        ...
        if (self.current_charge > self.max_charges or 
            (self.current_charge == self.max_charges and 
             self.mines_fired_in_current_charge >= self.mines_per_charge)):
            self.all_charges_used = True
            return
        
        if (self.auto_fire and self.mines_fired_in_current_charge < self.mines_per_charge):
            self.fire_timer += dt
    ...
    def _fire_mine(self) -> None:
        if not self.on_fire_mine: return
        screen = pygame.display.get_surface()
    ...
    def _update_entry_particles(self, dt: float) -> None:
        ...
        if self.is_entering and self.y > self.target_y:
            ...
            for _ in range(3):
                ...
                particle = ParticleDict(
                    x=self.x + self.w / 2 + random.uniform(-8, 8),  # Spawn próximo ao centro
                    ...
                    color=(150, 180, 255) if random.random() > 0.3 else (100, 150, 255)  # Azul claro
                )
        ...
        self.entry_particles = [
            ParticleDict(
                ...
                vy=p["vy"] * 0.98,
                lifetime=p["lifetime"] - dt * (2.0 if fade_factor < 1.0 else 1.0),  # Fade mais rápido
                size=max(0, p["size"] - dt * (4.0 if fade_factor < 1.0 else 2.0)),  # Encolher mais rápido durante fade
                color=p["color"]
            )
        ...
        if should_generate:
            for _ in range(particle_count):
                ...
                particle = ParticleDict(
                    ...
                    color=(100, 150, 255) if random.random() > 0.5 else (150, 180, 255)  # Azuis da mina
                )
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        glow_color, dark_glow_color = charge_colors.get(self.current_charge, ((100, 150, 255), (60, 100, 200)))
        
        # Durante entrada, aplicar efeito de transparência/brilho
        if self.is_entering:
            entry_progress = 1.0 - (self.entry_timer / 2.0)  # 0.0 a 1.0 (2 segundos total)
            # Intensificar cores durante entrada (efeito energético)
            glow_color = tuple(min(255, int(c * (1.0 + entry_progress * 0.5))) for c in glow_color)
            dark_glow_color = tuple(min(255, int(c * (1.0 + entry_progress * 0.3))) for c in dark_glow_color)
        ...
        pygame.draw.rect(surface, (45, 50, 70), (self.x - 5 + shake_x, base_y + 15 + shake_y, self.w + 10, 15), border_radius=3)
        
        # Corpo da base com tons azulados (aplicar tremor)
        pygame.draw.rect(surface, (80, 90, 120), (self.x + shake_x, base_y + shake_y, self.w, base_h))
        pygame.draw.rect(surface, (120, 140, 180), (self.x + shake_x, base_y + shake_y, self.w, 4)) # Highlight superior azulado
        ...
        pygame.draw.rect(surface, (35, 40, 55), (barrel_x + shake_x, turret_y - 20 + shake_y, barrel_w, barrel_h))
        
        # Bobinas de energia (Coils) no cano - brilham na cor da carga (aplicar tremor)
        for i in range(3):
            coil_y = turret_y - 15 + (i * 8)
            pygame.draw.rect(surface, dark_glow_color, (barrel_x - 2 + shake_x, coil_y + shake_y, barrel_w + 4, 4))
            # Centro brilhante
            pygame.draw.rect(surface, glow_color, (barrel_x + shake_x, coil_y + 1 + shake_y, barrel_w, 2))

        # Corpo principal da torre com tons azulados (aplicar tremor)
        pygame.draw.rect(surface, (100, 115, 140), (turret_x + shake_x, turret_y + shake_y, turret_w, turret_h), border_radius=5)
        
        # Detalhes de ventilação/tech azulados (aplicar tremor)
        pygame.draw.line(surface, (60, 70, 90), (turret_x + 5 + shake_x, turret_y + 10 + shake_y), (turret_x + turret_w - 5 + shake_x, turret_y + 10 + shake_y), 2)
        pygame.draw.line(surface, (60, 70, 90), (turret_x + 5 + shake_x, turret_y + 20 + shake_y), (turret_x + turret_w - 5 + shake_x, turret_y + 20 + shake_y), 2)
        ...
        panel_y = base_y + 8
        pygame.draw.rect(surface, (15, 20, 35), (panel_x + shake_x, panel_y + shake_y, panel_w, 8))
        ...
        if not self.all_charges_used:
            for i in range(self.mines_per_charge):
                ...
                total_width = self.mines_per_charge * led_size + (self.mines_per_charge - 1) * spacing
                ...
                pygame.draw.rect(surface, color, (led_x + shake_x, panel_y + 1 + shake_y, led_size, 6))
