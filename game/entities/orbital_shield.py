...
class OrbitalShield:
    ...
    def _spawn_spark(self):
        ...
        self.sparks.append({
            ...
            "vx": sx * 2, "vy": sy * 2,
            "life": 0.2,
            "color": random.choice([(0, 200, 255), (200, 255, 255), (255, 255, 255)])
        })
    ...
    def rect(self) -> pygame.Rect:
        ...
        return pygame.Rect(int(x - self.size/2), int(y - self.size/2), self.size, self.size)
    ...
    def draw(self, surface: pygame.Surface):
        ...
        for s in self.sparks:
            pygame.draw.line(surface, s["color"], (int(cx + s["x"]), int(cy + s["y"])), 
                             (int(cx + s["x"] + s["vx"]*0.02), int(cy + s["y"] + s["vy"]*0.02)), 1)
        ...
        pygame.draw.circle(surface, (0, 100, 200), (int(cx), int(cy)), int(self.size/2 + 2), 1)

        # 3. Núcleo Tecnológico (Quadrado rotacionado)
        self._draw_rotated_rect(surface, cx, cy, self.size * 0.6, self.rotation_inner, (50, 50, 60))
        self._draw_rotated_rect(surface, cx, cy, self.size * 0.4, -self.rotation_inner * 1.5, (0, 200, 255))
        
        # 4. Ponto central pulsante
        core_r = max(2, int(4 * pulse))
        pygame.draw.circle(surface, (255, 255, 255), (int(cx), int(cy)), core_r)

    def _draw_rotated_rect(self, surface: pygame.Surface, x: float, y: float, size: float, angle: float, color: tuple[int, int, int]):
