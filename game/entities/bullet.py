...
class Bullet:
    def __init__(self, x: float, y: float, damage: int = 10, piercing: bool = False, homing: bool = False, explosive: bool = False, low_ammo: bool = False):
        self.x, self.y = x, y
        self.w, self.h = 3, 10
        self.damage = damage
        self.dead = False
    ...
    def reset(self, x: float, y: float, damage: int = 10, piercing: bool = False, homing: bool = False, explosive: bool = False, low_ammo: bool = False):
        ...
        if not self.target or getattr(self.target, 'dead', True):
            ...
            if self.assigned_target_id is not None:
                for enemy in enemies:
                    if id(enemy) == self.assigned_target_id and not getattr(enemy, 'dead', True):
                        self.target = enemy
                        break
    ...
    def _draw_homing_bullet(self, surface: pygame.Surface):
        ...
        for i in range(-size, size + 1):
            ...
            pygame.draw.circle(surface, color, (int(pixel_x), int(pixel_y)), thickness // 2)
        ...
        pygame.draw.circle(surface, (150, 255, 150), (int(center_x), int(center_y)), thickness)
    ...
    def _draw_explosive_bullet(self, surface: pygame.Surface):
        ...
        pygame.draw.circle(surface, outer_color, (int(center_x), int(center_y)), radius + 1)
