...
class GiantMeteorBoss:
    ...
    def _generate_base_shape(self) -> list[tuple[float, float]]:
        ...
        for i in range(segs + 1):
            pts.append((i * self.w / segs, random.uniform(-8, 8)))
        for i in range(1, segs + 1):
            pts.append((self.w + random.uniform(-8, 8), i * self.h / segs))
        for i in range(segs, -1, -1):
            pts.append((i * self.w / segs, self.h + random.uniform(-8, 8)))
        for i in range(segs - 1, 0, -1):
            pts.append((random.uniform(-8, 8), i * self.h / segs))
        return pts
    ...
    def _apply_damage_cracks(self, hp_pct: float) -> list[tuple[float, float]]:
        ...
        while to_act > 0 and c_idx < total and att < total * 2:
            if c_idx not in self._crack_birth_stage:
                ...
                too_close = any(
                    (
                        (pos["start_x"] - self._all_crack_positions[a]["start_x"]) ** 2
        ...
        for i in range(min(num_active, total)):
            ...
            depth = (
                rng.uniform(0.20 + (age / 11.0) * 0.60, 0.35 + (age / 11.0) * 0.65)
                * max_d
            )
            crks.append(
                (
    ...
    def update(self, dt: float, entity_manager: "EntityManager") -> None:
        ...
        if stage != self._last_damage_stage:
            self._old_shape, self._target_shape = list(
                self._current_shape
            ), self._apply_damage_cracks(hp_pct)
            self._transition_timer, self._shake_timer, self._shake_intensity = (
                0.0,
                self._shake_duration,
                8.0 + stage * 0.7,
            )
        ...
        if self._target_shape:
            ...
            if t >= 1.0:
                self._current_shape, self._old_shape, self._target_shape = (
                    list(self._target_shape),
                    None,
                    None,
                )
        ...
        if self.state == "entering":
            ...
            if self.meteor_spawn_timer >= self.meteor_spawn_interval:
                self.meteor_spawn_timer = 0.0
                for _ in range(1 + int(stage * 1.2)):
    ...
    def _spawn_normal_meteor(self, em: "EntityManager") -> None:
        ...
        y = random.uniform(
            -10, 10
        )  # borda superior, sobrevive ao off-screen check em ambos os modos
        ...
        for _ in range(random.randint(*config.GIANT_METEOR_HIT_FRAGMENT_COUNT)):
            if self._crack_birth_stage and random.random() < 0.7:
                ...
                if self.is_side_scroll:
                    vx -= random.uniform(
                        80, 180
                    )  # bias para a esquerda (direção do scroll)
                ...
                size = random.randint(
                    mn + age * 2, min(mx, mn + int((mx - mn) * (age / 11.0) + 5))
                )
            else:
    ...
    def _build_death_fragment_specs(self) -> "tuple[MeteorSpec, ...]":
        ...
        for _ in range(random.randint(*config.GIANT_METEOR_DEATH_FRAGMENT_COUNT)):
            ...
            size = random.randint(
                ...
                config.GIANT_METEOR_FRAGMENT_MAX_SIZE + 25,
            )
