...
def apply_aim_spread(dx: float, dy: float, spread_degrees: float) -> Tuple[float, float]:
...
class BossAttackSystem:
    ...
    @staticmethod
    def spawn_aimed_meteor(boss_x: float, boss_y: float, boss_w: float, boss_h: float, 
                          target_x: float, target_y: float) -> "Meteor":
    ...
    @staticmethod
    def spawn_side_meteors(boss_x: float, boss_y: float, boss_w: float, boss_h: float,
                          player_x: float, player_y: float) -> List["Meteor"]:
        ...
        if left_length > 0:
            ...
            left_dx, left_dy = apply_aim_spread(left_dx, left_dy, Config.BOSS_SIDE_METEOR_AIM_SPREAD)
            ...
            # Renormaliza
            curved_length = math.sqrt(curved_left_dx * curved_left_dx + curved_left_dy * curved_left_dy)
            if curved_length > 0:
        ...
        if right_length > 0:
            ...
            # Aplica imprecisão antes da curva
            right_dx, right_dy = apply_aim_spread(right_dx, right_dy, Config.BOSS_SIDE_METEOR_AIM_SPREAD)
            ...
            # Renormaliza
            curved_length = math.sqrt(curved_right_dx * curved_right_dx + curved_right_dy * curved_right_dy)
            if curved_length > 0:
    ...
    @staticmethod
    def spawn_guided_meteor(boss_x: float, boss_y: float, boss_w: float, boss_h: float,
                           target_x: float, target_y: float) -> GuidedMeteor:
...
class BossCannon:
    ...
    def update_position(self, boss_x: float, boss_y: float, boss_width: float, boss_height: float) -> None:
