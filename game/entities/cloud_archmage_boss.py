...
from .mountain_mage import MountainStalagmite, MountainStalactite
...
_PALETTES: Final[Dict[str, Dict[str, tuple[int, int, int]]]] = {
    "normal": {
        "robe": (60, 45, 110), "metal": (100, 105, 115), "visor": (10, 10, 20),
        "core": (30, 25, 45), "hat": (45, 30, 85), "hat_hl": (80, 60, 150),
        "joint": (30, 30, 35), "gold": (212, 175, 55), "gola": (80, 70, 140),
    },
    "phase3": {
        "robe": (40, 10, 60), "metal": (60, 65, 80), "visor": (20, 0, 0),
...
_CHAR_TO_KEY: Final[Dict[str, str]] = {
    "H": "hat", "*": "hat_hl", "M": "metal", "V": "visor",
    "E": "core", "D": "joint", "O": "gold", "G": "gola",
}
...
class CloudArchmageBoss:
    ...
    def _bake_surface(self, pixelmap: List[str], pal: Dict[str, tuple[int, int, int]]) -> pygame.Surface:
        cols = len(pixelmap[0])
        rows = len(pixelmap)
        ...
        for r_idx, row in enumerate(pixelmap):
            for c_idx, char in enumerate(row):
                ...
                pygame.draw.rect(
                    surf,
                    pal[key],
                    (c_idx * self._SCALE, r_idx * self._SCALE, self._SCALE, self._SCALE),
                )
        return surf.convert_alpha()

    def _get_scaled_sprite(self, part: str, state_key: str, visual: float) -> pygame.Surface:
        ...
        if cache_key not in self._teleport_cache:
            ...
            self._teleport_cache[cache_key] = pygame.transform.scale(base, (w, h)).convert_alpha()
    ...
    def _sync_lerp_to_position(self) -> None:
        ...
        hat_surf_w  = self._sprites["hat"]["normal"].get_width()  if self._sprites.get("hat") else 0
        body_surf_w = self._sprites["body"]["normal"].get_width() if self._sprites.get("body") else 0
        arm_surf_w  = self._sprites["arm"]["normal"].get_width()  if self._sprites.get("arm") else 0
        ...
        self._hat_y  = by - 60.0
        self._eye_l_x = bx - 18.0;  self._eye_l_y = (self._hat_y + self._body_y + 20.0) / 2
        self._eye_r_x = bx + 10.0;  self._eye_r_y = self._eye_l_y
        self._l_arm_x = bx - 55.0;  self._l_arm_y = by + 20.0
        self._r_arm_x = bx + 55.0 - arm_surf_w; self._r_arm_y = by + 20.0
    ...
    def _update_lerp_physics(self, dt: float) -> None:
        ...
        self._l_arm_y += (by + 20.0 + math.cos(pt * 3.0) * 10.0 - self._l_arm_y) * _LERP_ARM * dt
        self._r_arm_x += (bx + 55.0 - arm_w - self._r_arm_x)    * _LERP_ARM * dt
        self._r_arm_y += (by + 20.0 + math.sin(pt * 3.0) * 10.0 - self._r_arm_y) * _LERP_ARM * dt
    ...
    def update(self, dt: float, player_pos: Tuple[float, float] | None = None) -> List[Any]:
        ...
        if self._shield_spawning:
            self._shield_spawn_t = min(1.0, self._shield_spawn_t + dt / self._shield_spawn_dur)
        ...
        if self._state not in (ArchmageState.APPEARING, ArchmageState.TELEPORT, ArchmageState.SLIDE, ArchmageState.DEFEATED):
        ...
        if self._state != ArchmageState.TELEPORT:
            self._teleport_visual = min(1.0, self._teleport_visual + _TELEPORT_FADE_SPEED * dt)
    ...
    def rect(self) -> pygame.Rect:
        ...
        if self._shield_active or self._shield_spawning:
            r = int(self.SHIELD_RADIUS * (self._shield_spawn_t if self._shield_spawning else 1.0))
    ...
    def collision_circle(self) -> tuple[float, float, float]:
        ...
        if self._shield_active or self._shield_spawning:
            r = self.SHIELD_RADIUS * (self._shield_spawn_t if self._shield_spawning else 1.0)
            return cx, cy, r
    ...
    def _update_idle(self, dt: float, hp_ratio: float) -> None:
        ...
        if hp_ratio > self.PHASE2_THRESHOLD:
            self._state = random.choice((ArchmageState.PHASE1_TWIN_ORBS, ArchmageState.PHASE1_EXPANSION))
        elif hp_ratio > self.PHASE3_THRESHOLD:
            self._state = random.choice((ArchmageState.PHASE2_MAZE_ABSORB, ArchmageState.PHASE2_SIPHON))
        else:
            self._state = random.choice((ArchmageState.PHASE1_TWIN_ORBS, ArchmageState.PHASE2_MAZE_ABSORB))
    ...
    def _update_maze(self, dt: float, player_pos: Tuple[float, float] | None) -> List[Any]:
        ...
        if last_tick != current_tick and self._state_timer > 1.2:
            ...
            self._active_telegraphs.append({
                ...
                "timer":         0.9,
                "charge":        0.0,
                "target_y":      player_pos[1] if player_pos else Config.SCREEN_HEIGHT / 2,
            })
        ...
        for t in self._active_telegraphs[:]:
            ...
            if t["timer"] <= 0.0:
                if t["is_stalactite"]:
                    ...
                    spawned.append(MountainStalagmite(t["x"], float(Config.SCREEN_HEIGHT) + 10.0, t["target_y"]))
    ...
    def _update_teleport(self, dt: float) -> None:
        ...
        if self._state_timer > self._TELEPORT_HALF:
            # Phase A: fade out
            self._teleport_visual = max(0.0, self._teleport_visual - _TELEPORT_FADE_SPEED * dt)
        else:
    ...
    def _update_twin_orbs(self, dt: float, player_pos: Tuple[float, float] | None) -> None:
        if not player_pos:
            return
        self._state_timer -= dt
    ...
    def _update_expansion(self, dt: float, player_pos: Tuple[float, float] | None) -> None:
        self._state_timer -= dt
        cx = self.x + self.w / 2
        cy = self.y + self.h / 2
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        hp_ratio  = self.health / self.max_health
        state_key = "flash" if self._hit_flash > 0.0 else ("normal" if hp_ratio > 0.3 else "phase3")
    ...
    def _draw_shield(self, surface: pygame.Surface) -> None:
        ...
        for ring in self._shield_rings:
            ...
            rc = ring_r + 2
            pygame.draw.circle(ring_surf, (0, 210, 255, ring_alpha), (rc, rc), ring_r, 2)
        ...
        if not self._shield_spawning:
            ...
            pygame.draw.rect(surface, (0, 220, 255), (bx, by, int(bar_w * hp_pct), bar_h))
    ...
    def _draw_health_bar(self, surface: pygame.Surface) -> None:
        ...
        pygame.draw.rect(surface, color,           (int(bx), int(by), int(bar_w * hp_ratio), bar_h))
        pygame.draw.rect(surface, (120, 120, 180), (int(bx), int(by), bar_w, bar_h), 1)

    def _draw_telegraph_marker(self, surface: pygame.Surface, t: dict[str, Any]) -> None:
        mr = int(14 + t["charge"] * 18)
        diam = mr * 4
        ms = pygame.Surface((diam, diam), pygame.SRCALPHA)
        mc = diam // 2
        alpha = int(100 + t["charge"] * 120)
        color = (255, 235, 140, alpha) if not t["is_stalactite"] else (160, 220, 255, alpha)
    ...
    def _draw_flowing_mantle(self, surface: pygame.Surface, state_key: str) -> None:
        ...
        for i in range(4):
            offset_x = i * 12 + 6
            wave = math.sin(self._mantle_timer * 4.0 * _FLAP_FREQ[i] + _FLAP_PHASE[i]) * _FLAP_AMP[i]
            pts = [
    ...
    def _draw_orbs(self, surface: pygame.Surface) -> None:
        ...
        for i, orb in enumerate(self._orbs):
            ...
            surface.blit(rotated, (ox - rotated.get_width() // 2, oy - rotated.get_height() // 2))
    ...
    def on_hit(self, damage: int, hit_x: float, hit_y: float) -> "HitResult":
        ...
        if self.dead:
            return HitResult(killed=True, points=5000, explosion_size=100, sound=hit_sounds.EXPLOSION_ALIEN)
