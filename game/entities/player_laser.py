...
LIGHTNING_REGENERATE_INTERVAL = 0.05  # Regenerar pontos a cada 50ms para efeito dinâmico
...
class PlayerLaser:
    ...
    def _update_origin_position(self) -> None:
        ...
        if self.ship is not None and self.ball_index >= 0:
            ...
            angle = self.ship.orbital_angle + (self.ball_index * 2 * math.pi / self.ship.num_orbital_balls)
            self.x = self.ship.x + self.ship.w / 2 + math.cos(angle) * self.ship.orbital_radius
            self.y = self.ship.y + self.ship.h / 2 + math.sin(angle) * self.ship.orbital_radius
    ...
    def update(self, dt: float):
        ...
        if self.state == "alive":
            if self.timer >= self.lifetime:
                ...
                for i in range(num_particles):
                    pos_on_line = start_pos + line_vec * (
                        i * LIGHTNING_SEGMENT_LENGTH + random.uniform(0, LIGHTNING_SEGMENT_LENGTH)
                    )
            ...
            self.death_particles = [
                p for p in self.death_particles 
        ...
        if self.state == "alive" and self.w > 0:
            ...
            if width > 0 and height > 0:
                ...
                for i in range(len(self.lightning_points) - 1):
                    start = (self.lightning_points[i][0] - offset_x, 
                            self.lightning_points[i][1] - offset_y)
                    end = (self.lightning_points[i + 1][0] - offset_x, 
                ...
                surface.blit(glow_surface, (int(min_x), int(min_y)), special_flags=pygame.BLEND_RGBA_ADD)
                
                # Desenhar núcleo do raio diretamente na superfície principal
                for i in range(len(self.lightning_points) - 1):
                    start = self.lightning_points[i]
                    end = self.lightning_points[i + 1]
                    pygame.draw.line(surface, colors.WHITE, start, end, max(2, int(self.w)))
                ...
                if random.random() < SPARK_CHANCE_PER_FRAME:
                    for point in self.lightning_points[1:-1]:
                        if random.random() < SPARK_CHANCE_PER_POINT:
                            ...
                            spark_vec = pygame.math.Vector2(spark_length, 0).rotate(spark_angle)
                            spark_end = (point[0] + spark_vec.x, point[1] + spark_vec.y)
                            pygame.draw.line(surface, (200, 240, 255), point, spark_end, 1)
