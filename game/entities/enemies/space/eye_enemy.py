...
from ...projectiles.eye_laser import EyeLaser
from ...effects.particle_types import ChargingParticle
...
class EyeEnemy:
            ...
            if not self.charging_particles or self.timer <= 0:
                ...
                if self.locked_player_pos is not None:
                    ...
                    if length == 0:
                        ...
                        dir_x /= length
                        dir_y /= length
                        max_screen_distance = math.hypot(
    ...
    def _initialize_charging_particles(self):
        ...
        for _ in range(25):
            ...
            particle_color: Tuple[int, int, int] = random.choice(
    ...
    def _draw_dashed_line(
        ...
        while current_distance < length:
            ...
            end_segment = pygame.math.Vector2(x1, y1) + dl * min(
                current_distance + self.aim_dash_length, length
            )
            pygame.draw.line(
                surface,
                color,
                (int(start_segment.x), int(start_segment.y)),
                (int(end_segment.x), int(end_segment.y)),
                width,
            )
            current_distance += self.aim_dash_length + self.aim_gap_length

    def _draw_pixel_eye(
        self, surface: pygame.Surface, player_x: float | None, player_y: float | None
    ):
            ...
            for px in range(0, self.w, pixel_size):
                ...
                if norm_x * norm_x + norm_y * norm_y <= 1.0:
                    ...
                    pygame.draw.rect(
                        surface,
                        color,
                        (base_x + px, base_y + py, pixel_size, pixel_size),
                    )
        ...
        for i in range(num_veins):
            ...
            for dist in range(int(start_dist), int(end_dist), pixel_size):
                ...
                vein_draw_color = tuple(
                    int(vein_color[i] * vein_intensity) for i in range(3)
                )

                # Veias mais grossas e escuras quando carregando
                vein_thickness = (
                    pixel_size * 2
                    if self.state in ("charging", "aiming")
                    else pixel_size
                )
                pygame.draw.rect(
                    surface,
                    vein_draw_color,
                    (int(vein_x), int(vein_y), vein_thickness, vein_thickness),
                )
            ...
            for px in range(-iris_radius, iris_radius, pixel_size):
                if px * px + py * py <= iris_radius * iris_radius:
                    ...
                    if dist < 0.3:
                        ...
                        mix = (dist - 0.3) / 0.4
                        color = tuple(
                            int(pupil_color[i] * (1 - mix) + iris_color[i] * mix)
                            for i in range(3)
                        )
                    else:
                        color = iris_color
        ...
        if self.state in ("charging", "ready", "aiming"):
            ...
            for py in range(-glow_radius, glow_radius, pixel_size):
                for px in range(-glow_radius, glow_radius, pixel_size):
                    dist_sq = px * px + py * py
                    if (
                        dist_sq <= glow_radius * glow_radius
                        and dist_sq > iris_radius * iris_radius
                    ):
    ...
    def draw(
        ...
        if self.state in ("aiming", "charging") and self.locked_player_pos:
            ...
            if length == 0:
                ...
                extended_end_pos = (
                    start_pos[0] + dir_x * max_screen_distance * 2,
                    start_pos[1] + dir_y * max_screen_distance * 2,
                )
            ...
            self._draw_dashed_line(
                ...
                (255, 60, 60),
                2,
                anim_offset=self.aim_anim_offset,
            )
