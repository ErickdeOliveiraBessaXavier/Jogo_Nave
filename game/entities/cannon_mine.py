...
class CannonMine:
    ...
    def update(self, dt: float) -> None:
        ...
        if self.state == MineState.FLYING:
            ...
            if self.time_elapsed >= self.travel_time:
                ...
                parabolic_offset = -4 * MineConfig.ARC_HEIGHT * progress * (progress - 1)
    ...
    def hit_tracking_set(self) -> Set[int]:
        ...
        return (self.hit_enemies if self.state == MineState.EXPLODING 
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        if self.state == MineState.EXPLODING:
            ...
            if radius > 0:
                ...
                pygame.draw.circle(explosion_surf, (*base_color, alpha), (radius, radius), radius)
                surface.blit(explosion_surf, (int(self.x - radius), int(self.y - radius)))
            ...
            for i in range(4):
                ...
                if emerge_factor > 0.1:  # Só desenha quando está emergindo
                    ...
                    p2 = (int(base_x + math.cos(perp_angle) * base_width), 
                          int(base_y + math.sin(perp_angle) * base_width))  # Base esquerda
                    p3 = (int(base_x + math.cos(perp_angle + math.pi) * base_width), 
                          int(base_y + math.sin(perp_angle + math.pi) * base_width))  # Base direita
            ...
            if self.state == MineState.LANDED:
                ...
                if progress > 0:
                    ...
                    for i in range(points_to_draw):
                        angle = (i / total_points) * 2 * math.pi - math.pi / 2  # Começar do topo
                        point_x = center_x + math.cos(angle) * arc_radius
                        point_y = center_y + math.sin(angle) * arc_radius
                        pygame.draw.circle(surface, spike_color, (int(point_x), int(point_y)), 2)
            
            elif self.state == MineState.ARMED:
                # Área de perigo - círculo azul transparente
                danger_surf = pygame.Surface((self.explosion_radius * 2, self.explosion_radius * 2), pygame.SRCALPHA)
                pygame.draw.circle(danger_surf, (*base_color, 30), 
                                 (int(self.explosion_radius), int(self.explosion_radius)), 
                                 int(self.explosion_radius))
                pygame.draw.circle(danger_surf, (*base_color, 100), 
                                 (int(self.explosion_radius), int(self.explosion_radius)), 
                                 int(self.explosion_radius), 1)
                surface.blit(danger_surf, (self.x - self.explosion_radius, self.y - self.explosion_radius))