...
class EyeLaser:
    def __init__(self, x: float, y: float, target_x: float, target_y: float, max_w: int = 8, lifetime: float = 1.2):
        self.x = x
        self.y = y
    ...
    def draw(self, surface: pygame.Surface):
        if self.state == "alive" and self.w > 0:
            ...
            glow_surface2 = pygame.Surface((surface.get_width(), surface.get_height()), pygame.SRCALPHA)
            pygame.draw.line(
                glow_surface2, 
            ...
            for p in self.death_particles:
                if p["size"] > 0:
                    pygame.draw.circle(surface, p["color"], (int(p["pos"].x), int(p["pos"].y)), int(p["size"]))