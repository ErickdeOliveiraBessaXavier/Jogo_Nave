...
class MountainStalagmite:
    ...
    def __init__(self, x: float, ground_y: float, target_y: float) -> None:
        ...
        self._secondary_left_target:  int = max(20, int(self._target_height * self.SECONDARY_MAX_RATIO))
        self._secondary_right_target: int = max(20, int(self._target_height * self.SECONDARY_MAX_RATIO * 0.82))
    ...
    def causes_damage(self) -> bool:
        return not self.dead and self._state in (_StalagState.RISING, _StalagState.ACTIVE)
    ...
    def get_ship_contact_hitboxes(self) -> tuple[pygame.Rect, ...]:
        if self.dead or self._state is _StalagState.SHATTERING or self._collision_bounds_rect.width <= 0:
            return self._EMPTY_HITBOXES
        return (self._collision_bounds_rect,)

    def get_collision_mask_data(self) -> tuple[pygame.mask.Mask, tuple[int, int]] | None:
        if self.dead or self._state is _StalagState.SHATTERING or self._collision_mask is None:
            return None
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        if left_h >= 4:
            self._draw_flat_spike(
                surface, cx=cx - int(main_hw * 0.95), base_y=base_y,
                height=max(4, left_h), half_w=left_hw,
                phase_seed=self._shape_phase + 1.2,
                color=c_left, edge=c_edge, forced_lean_dir=-1,
            )

        if self._main_delay_timer >= 0.0:
            self._draw_flat_spike(
                surface, cx=cx, base_y=base_y, height=height,
                half_w=main_hw, phase_seed=self._shape_phase,
                color=c_center, edge=c_edge,
            )

        if right_h >= 4:
            self._draw_flat_spike(
                surface, cx=cx + int(main_hw * 0.88), base_y=base_y,
                height=max(4, right_h), half_w=right_hw,
                phase_seed=self._shape_phase - 0.9,
                color=c_right, edge=c_edge, forced_lean_dir=1,
            )
    ...
    def _update_secondary_timers(self, dt: float) -> None:
        ...
        if self._secondary_left_timer >= 0.0:
            progress = _clamp(self._secondary_left_timer / self.SECONDARY_RISE_TIME, 0.0, 1.0)
            self._secondary_left_height = max(1.0, self._secondary_left_target * _ease_out_cubic(progress))

        self._secondary_right_timer += dt
        if self._secondary_right_timer >= 0.0:
            progress = _clamp(self._secondary_right_timer / self.SECONDARY_RISE_TIME, 0.0, 1.0)
            self._secondary_right_height = max(1.0, self._secondary_right_target * _ease_out_cubic(progress))
    ...
    def _spawn_fragments(self) -> None:
        ...
        for _ in range(_FRAGMENT_COUNT):
            ...
            self._fragments.append(_StalagmiteFragment(
    ...
    def _draw_fragments(self, surface: pygame.Surface) -> None:
        for fragment in self._fragments:
            ...
            pygame.draw.circle(dust_surf, (*fragment.color, dust_alpha), (dust_size, dust_size), dust_size)
            pygame.draw.circle(
                dust_surf, (193, 154, 131, int(dust_alpha * 0.7)),
                (dust_size + 1, dust_size - 1), max(2, dust_size // 2),
            )
            dust_pos = (
            ...
            pygame.draw.rect(frag_surf, (70, 44, 34, min(255, alpha + 20)), frag_rect, width=1, border_radius=br)
            pygame.draw.line(
                frag_surf, (215, 178, 154, int(alpha * 0.55)),
                (frag_rect.left + 2, frag_rect.centery - 1),
                (frag_rect.right - 3, frag_rect.centery - 2), width=1,
            )
            rotated = pygame.transform.rotate(frag_surf, math.degrees(fragment.angle))
            surface.blit(rotated, rotated.get_rect(center=(int(fragment.x), int(fragment.y))))
    ...
    def _build_visual_spike_polygons(self) -> _SpikePolygons:
        ...
        if left_h >= 4:
            polygons.append(self._build_flat_spike_pts(
                cx=cx - int(main_hw * 0.95), base_y=base_y, height=max(4, left_h),
                half_w=left_hw, phase_seed=self._shape_phase + 1.2, forced_lean_dir=-1,
            ))

        if self._main_delay_timer >= 0.0:
            polygons.append(self._build_flat_spike_pts(
                cx=cx, base_y=base_y, height=height,
                half_w=main_hw, phase_seed=self._shape_phase,
            ))

        if right_h >= 4:
            polygons.append(self._build_flat_spike_pts(
                cx=cx + int(main_hw * 0.88), base_y=base_y, height=max(4, right_h),
                half_w=right_hw, phase_seed=self._shape_phase - 0.9, forced_lean_dir=1,
            ))

        return polygons

    def _build_flat_spike_pts(
        self, cx: int, base_y: int, height: int, half_w: int,
        phase_seed: float, forced_lean_dir: int | None = None,
    ) -> list[tuple[int, int]]:
        ...
        return self._build_spike_pts(cx, base_y, height, half_w, lean_dir, lean_top, lean_mid, notch_cut)
    ...
    def _draw_flat_spike(
        ...
        cx: int, base_y: int, height: int, half_w: int,
        phase_seed: float, color: tuple[int, int, int], edge: tuple[int, int, int],
        forced_lean_dir: int | None = None,
    ) -> None:
        pts = self._build_flat_spike_pts(cx, base_y, height, half_w, phase_seed, forced_lean_dir)
        pygame.draw.polygon(surface, color, pts)
        pygame.draw.polygon(surface, edge,  pts, width=2)

    def _draw_shadow_stalactite(
        self, surface: pygame.Surface, offset_x: float, scale: float, alpha: int,
        color_base: tuple[int, int, int], color_mid: tuple[int, int, int],
        ...
        y_n2  = base_y - int(height * 0.46)
        y_n3  = base_y - int(height * 0.20)
        hw_base, hw_n3, hw_n2, hw_n1 = half_W, max(int(half_W * .80), 4), max(int(half_W * .54), 3), max(int(half_W * .30), 2)
...
class MountainMage:
    ...
    def __init__(self, x: float | None = None, y: float | None = None) -> None:
        ...
        self.x: float = float(x if x is not None else random.randint(90, max(110, int(screen_w) - 140)))
        self.y: float = float(y if y is not None else random.randint(70, max(90, int(screen_h * 0.24))))
        ...
        self._last_player_pos:   tuple[float, float] = (self.x + self.w / 2, self.y + self.h)
    ...
    def _update_state(
        ...
        if self._state is _MageState.IDLE:
            ...
            self._telegraph_charge = _clamp(1.0 - self._state_timer / warning_duration, 0.0, 1.0)
            self._state_timer -= dt
            if self._state_timer <= 0.0:
    ...
    def _update_movement(self, dt: float) -> None:
        ...
        drift_speed = self.DRIFT_SPEED * (self.TELEGRAPH_SLOWDOWN if self._state is _MageState.TELEGRAPH else 1.0)
    ...
    def _draw_orb(self, surface: pygame.Surface) -> None:
        ...
        pygame.draw.circle(glow_surf, (*self._orb_base_color, glow_a), (glow_r, glow_r), glow_r)
        ...
        pygame.draw.circle(surface, colors.WHITE, (int(orb_x) - 3, int(orb_y) - 3), max(2, self.ORB_RADIUS // 4))
