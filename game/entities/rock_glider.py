...
class RockGlider(Meteor):
    ...
    def reset(
        ...
        self._rock_center: tuple[float, float] = (self.x + self.size, self.y + self.size)
    ...
    def _build_geometry(self) -> tuple[list[tuple[int, int]], pygame.Rect, int, int]:
        ...
        rock_span_screen = max(x for x, _ in rock_points) - min(x for x, _ in rock_points)
    ...
    def _update_collision_regions(self) -> None:
        ...
        thruster_zone = pygame.Rect(body_rect.left, body_rect.bottom, body_rect.width, 14)
        self._bot_hit_rect = body_rect.union(left_arm).union(right_arm).union(thruster_zone)
    ...
    def update(self, dt: float, is_side_scroll: bool = False):
        ...
        if not self._bot_destroyed:
            ...
            self._eye_scan_offset = int(round(math.sin(self._time * self._eye_scan_speed)))
    ...
    def draw(self, screen: pygame.Surface):
        ...
        self._rock_center = (float(sum(x for x, _ in rock_points) / len(rock_points)), float(sum(y for _, y in rock_points) / len(rock_points)))
        ...
        if not self._bot_destroyed:
            ...
            eye_h = eye_size if self._eye_blink_remaining <= 0.0 else max(1, eye_size // 3)
            ...
            if eye_h > 1:
                ...
                pygame.draw.rect(
                    screen,
                    colors.BLACK,
                    (left_eye_x + (eye_size - pupil_w) // 2, eye_y + (eye_h - pupil_h) // 2, pupil_w, pupil_h),
                )
                pygame.draw.rect(
                    screen,
                    colors.BLACK,
                    (right_eye_x + (eye_size - pupil_w) // 2, eye_y + (eye_h - pupil_h) // 2, pupil_w, pupil_h),
                )
            ...
            right_nozzle = pygame.Rect(body_rect.right - nozzle_w - 1, nozzle_y, nozzle_w, nozzle_h)
            ...
            for cx_ring, side_phase in (
                ...
                for i in range(ring_count):
                    phase = ((self._time * ring_speed) + (i / ring_count) + side_phase) % 1.0
