...
class SpikeBoss:
    ...
    def __init__(
        self, x: float, y: float, health: int = Config.SPIKE_BOSS_HEALTH
    ):
        # Posição e tamanho
        self.w = 120
        self.h = 100
    ...
    def _update_eye_behavior(self, dt: float):
        ...
        if self.eye_mode == "tracking":
            if self.eye_mode_timer >= Config.SPIKE_BOSS_EYE_TRACK_DURATION:
                ...
                self.eye_mode_timer = 0.0
                self.eye_frenetic_timer = 0.0
                self.eye_frenetic_direction = random.choice([-1, 1])  # Começa esquerda ou direita
    ...
    def _update_waves(self, dt: float, spikes: List[Spike]):
        ...
        if self.should_launch_all_spikes:
            ...
            for spike in spikes:
                if spike.state == 'attached':
                    spike.time_until_attack = random.uniform(0.0, 0.2)  # Lançamento quase instantâneo
        ...
        # Iniciar nova onda
            ...
            self.current_wave_size = random.randint(
                Config.SPIKE_WAVE_MIN_SIZE,
                Config.SPIKE_WAVE_MAX_SIZE
            )
    ...
    def draw(self, surface: pygame.Surface):
        ...
        pulse_color = (pulse_intensity, pulse_intensity // 2, pulse_intensity // 2) if self.frenzy_mode else (pulse_intensity, pulse_intensity, pulse_intensity + 55)
        ...
        if self.proximity_telegraph_active:
            ...
            text_rect = warning_text.get_rect(center=(boss_center_x, boss_center_y - 80))
        ...
        # Desenhar onda de proximidade se ativa
            ...
            # Onda externa
            pygame.draw.circle(surface, outer_color, (boss_center_x, boss_center_y), 
                             int(self.proximity_wave_radius), width=3)
            
            # Onda média
            if self.proximity_wave_radius > 20:
                pygame.draw.circle(surface, outer_color, (boss_center_x, boss_center_y), 
                                 int(self.proximity_wave_radius * 0.7), width=2)
    ...
    def _draw_health_bar(self, surface: pygame.Surface, x: int, y: int):
        ...
        health_color = colors.GREEN if health_ratio > 0.5 else colors.YELLOW if health_ratio > 0.25 else colors.RED
        pygame.draw.rect(surface, health_color, (bar_x, bar_y, health_width, bar_height))

        # Borda
        pygame.draw.rect(surface, colors.WHITE, (bar_x, bar_y, bar_width, bar_height), width=1)
