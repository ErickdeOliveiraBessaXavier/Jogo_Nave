...
class GolemMine:
        ...
        if self._phase == "landing":
            ...
            if self._fuse_timer >= self.FUSE_TIME:
                ...
                for i in range(self.EXPL_SHARDS):
                    ...
                    spawned.append(RockShard(
                        self.x, self.y, angle_deg,
                        speed_mult=1.4,
                        color=(255, 100, 60),
                    ))
                self.dead = True
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        fuse_ratio = self._fuse_timer / self.FUSE_TIME if self._phase == "armed" else 0.0
        ...
        pygame.draw.circle(glow_surf, (*self._COLOR_RING, glow_alpha),
        ...
        pygame.draw.rect(surface, body_color, (cx - S,   cy - S*3, S*2, S*6))  # vertical
        pygame.draw.rect(surface, body_color, (cx - S*3, cy - S,   S*6, S*2))  # horizontal
        pygame.draw.rect(surface, body_color, (cx - S*2, cy - S*2, S*4, S*4))  # centro
...
class RockShard:
    ...
    def update(self, dt: float) -> None:
        ...
        if (self.y > screen_h + 40 or self.y < -40
                or self.x < -40 or self.x > screen_w + 40):
            self.dead = True
...
# ============================================================================
# PEDRA ORBITAL — usada no ataque EARTH (nao causa dano enquanto orbita)
# ============================================================================

# Paleta de pedra para OrbitalRock — três tons do Golem (B, C, D) para profundidade
_ORBITAL_ROCK_COLORS: list[Tuple[int, int, int]] = [
    (123, 107, 85),   # B — marrom escuro
    (156, 133, 102),  # C — marrom médio
    (184, 157, 122),  # D — marrom claro (areia)
]
...
class OrbitalRock:
    ...
    def update(
        ...
        if self.phase == "pulling":
            ...
            if (self.y > self.screen_h + 80
                    or self.x < -80 or self.x > self.screen_w + 80):
                self.dead = True
...
# Paleta de cores pixel-art — espelha o HTML de referencia
...
# Mapa de pixels (19 colunas x 22 linhas) — espelha o map[] do JS
_PIXEL_MAP: list[list[str | None]] = [
    [None,None,None,None,None,None,None,None,None,"F",None,None,None,None,None,None,None,None,None],
    [None,None,None,None,None,None,None,None,"E","D","E",None,None,None,None,None,None,None,None],
    [None,None,None,None,None,None,None,"D","C","B","C","D",None,None,None,None,None,None,None],
    [None,None,None,None,None,None,"E","D","C","B","C","D","E",None,None,None,None,None,None],
    [None,None,None,None,None,"F","E","D","C","B","C","D","E","F",None,None,None,None,None],
    [None,None,None,None,"E","D","D","C","C","B","C","C","D","D","E",None,None,None,None],
    [None,None,None,"D","C","C","C","B","B","A","B","B","C","C","C","D",None,None,None],
    # linha 7: colunas 7-11 sao o visor — desenhadas separadamente em _draw_eye
    [None,None,"E","D","C","C","B",None,None,None,None,None,"B","C","C","D","E",None,None],
    [None,None,"E","D","C","C","B","B","B","B","B","B","B","C","C","D","E",None,None],
    [None,"E","D","C","C","B","B","B","B","B","B","B","B","B","C","C","D","E",None],
    ["E","D","C","C","B","B","B","B","B","A","B","B","B","B","B","C","C","D","E"],
    ["D","C","B","B","B","B","B","B","A","A","A","B","B","B","B","B","B","C","D"],
    ["C","B","B","B","B","B","B","A","A","A","A","A","B","B","B","B","B","B","C"],
    [None,"A","B","B","B","B","A","A","B","B","B","A","A","B","B","B","B","A",None],
    [None,None,"A","B","B","A","A","B","B","B","B","B","A","A","B","B","A",None,None],
    [None,None,None,"A","B","B","B","B","B","B","B","B","B","B","B","A",None,None,None],
    [None,None,None,None,"A","B","B","B","B","B","B","B","B","B","A",None,None,None,None],
    [None,None,None,None,None,"A","B","B","B","B","B","B","B","A",None,None,None,None,None],
    [None,None,None,None,None,None,"A","B","B","B","B","B","A",None,None,None,None,None,None],
    [None,None,None,None,None,None,None,"A","B","B","B","A",None,None,None,None,None,None,None],
    [None,None,None,None,None,None,None,None,"A","B","A",None,None,None,None,None,None,None,None],
    [None,None,None,None,None,None,None,None,None,"A",None,None,None,None,None,None,None,None,None],
]
_PIXEL_ROWS = len(_PIXEL_MAP)      # 22
_PIXEL_COLS = len(_PIXEL_MAP[0])   # 19

# Constantes do olho
_EYE_ROW        = 7    # linha onde fica o visor
_EYE_ROW_ABOVE  = 6    # linha que se afasta para CIMA com eye_growth
_EYE_ROW_BELOW  = 8    # linha que se afasta para BAIXO
_EYE_COL_START  = 7    # coluna inicial do visor (5 pixels de largura)
_EYE_COL_END    = 11   # coluna final (inclusive)
...
class StoneGolemBoss:
    ...
    def __init__(
        ...
        logger.debug(
            ...
            self.x, self.y, self.max_health, difficulty_multiplier,
        )
    ...
    def _change_fsm(self, new_state: str) -> None:
        ...
        if new_state == "EARTH_PULL":
            ...
            for _ in range(15):
                ...
                self._orbital_rocks.append(OrbitalRock(
                    screen_w=self._screen_w, screen_h=self._screen_h,
                    orbit_cx=cx, orbit_cy=cy,
                    target_rx=rx, target_ry=ry,
                    orbit_angle_start=angle,
                    rock_size=size, color=color, S=S,
                ))
    ...
    def update(
        ...
        _anchored = {
            "CHARGE","FIRE",
            "EARTH_SHAKE","EARTH_PULL","EARTH_ORBIT","EARTH_FIRE",
            "ORB_SPAWN","ORB_HOLD","ORB_FIRE",
            "SWEEP_CHARGE","SWEEP_FIRE",
        }
        target_float = (
            0.0 if self.fsm_state in _anchored
            else round(math.sin(self._time * 2.5) * 12)
        )
        self._current_float_y += (
            (target_float - self._current_float_y) * min(1.0, 6.0 * dt)
        )
        ...
        self._charge_particles = [
            p for p in self._charge_particles if not p.dead
        ]
    ...
    def _run_fsm(
        ...
        # ── SCAN (idle) ───────────────────────────────────────────────────────
            ...
            if t > 2.0:
                r = random.random()
                if   r < 0.25: self._change_fsm("OPENING")
                elif r < 0.50: self._change_fsm("EARTH_SHAKE")
                elif r < 0.75: self._change_fsm("ORB_SPAWN")
                else:          self._change_fsm("SWEEP_CHARGE")
        ...
        # ── TERRA ─────────────────────────────────────────────────────────────
            ...
            all_gone = not self._orbital_rocks or all(r.phase == "fired" and r.dead for r in self._orbital_rocks)
        ...
        # ── ORBES ─────────────────────────────────────────────────────────────
            ...
            if int(t * 2) != int((t - dt) * 2):
                for angle_deg in range(0, 360, 45):
                    new_shards.append(
                        RockShard(px, py, angle_deg, speed_mult=1.2,
        ...
        # ── SWEEP ─────────────────────────────────────────────────────────────
            ...
            if t > delay:
                ...
                if bucket not in self._shards_fired_at:
                    ...
                    new_shards.append(
                        RockShard(px, py, math.degrees(shoot_angle),
                                  speed_mult=1.3, color=_C["SWEEP_BEAM"])
                    )
        ...
        for row_idx, row in enumerate(_PIXEL_MAP):
            for col_idx, key in enumerate(row):
                if key is None:
                    continue
                if (row_idx == _EYE_ROW
                        and _EYE_COL_START <= col_idx <= _EYE_COL_END):
                    continue
    ...
    def _draw_eye(
        ...
        ox: int, oy: int, S: int, eye_offset: int,
    ) -> None:
        state = self.fsm_state
        ...
        pygame.draw.rect(surface, bg_col,
                         (visor_x, visor_y, S * 5, visor_h))

        iris_x = visor_x + S + self._scan_step * S
        pygame.draw.rect(surface, iris_col,
                         (iris_x, visor_y, S * 3, visor_h))

        reflex_x = visor_x + S * 2 + self._scan_step * S
        reflex_y = visor_y + 1
        pygame.draw.rect(surface, _C["PUPIL"],
                         (reflex_x, reflex_y, S, S))
    ...
    def _draw_thruster(
        ...
        for i in range(rings):
            ...
            if w < S:
                continue
            if   phase < 0.15: color = (255, 255, 255)
            elif phase < 0.50: color = (157, 212, 240)
            else:              color = (91,  159, 200)
            try:
    ...
    def _draw_sweep_cone(self, surface: pygame.Surface) -> None:
        ...
        alpha    = (int(38 + math.sin(self._time * 20) * 12)
                    if self.fsm_state == "SWEEP_CHARGE" else 40)
    ...
    def _draw_sweep_beam(self, surface: pygame.Surface) -> None:
        ...
        pygame.draw.line(halo_surf, (r, g, b, 77),
                         (int(px), int(py)),
                         (int(ex), int(ey)),
                         self.SCALE * 3)
    ...
    def _draw_charge_core(self, surface: pygame.Surface) -> None:
        ...
        S      = self.SCALE
        t      = self.fsm_ticks
        color  = (_C["EYE_IRIS_LASER"] if self.fsm_state == "CHARGE"
                  else _C["EYE_IRIS_SWEEP"])
        ...
        if idx == 0:
            pygame.draw.rect(surface, color,
                             (int(px)-S//2, int(py)-S//2, S, S))
        elif idx == 1:
            pygame.draw.rect(surface, color,
                             (int(px)-S//2, int(py)-S*2, S, S*4))
            pygame.draw.rect(surface, color,
                             (int(px)-S*2,  int(py)-S//2, S*4, S))
            pygame.draw.rect(surface, _C["PUPIL"],
                             (int(px)-S//2, int(py)-S//2, S, S))
        else:
            pygame.draw.rect(surface, color,
                             (int(px)-S//2, int(py)-S*3, S, S*6))
            pygame.draw.rect(surface, color,
                             (int(px)-S*3,  int(py)-S//2, S*6, S))
            pygame.draw.rect(surface, color,
                             (int(px)-S*2,  int(py)-S*2,  S*4, S*4))
            for i in range(4):
    ...
    def _draw_health_bar(
        ...
        if fw > 0:
            pygame.draw.rect(surface, hp_color,
                             (bar_x, bar_y, fw, bar_h), border_radius=3)
