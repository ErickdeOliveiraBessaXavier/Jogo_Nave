...
class BlackHole:
    ...
    def __init__(self, x: float, y: float, duration: float):
        ...
        # Movimento
        self.speed_y = config.BLACK_HOLE_SPEED_Y  # Velocidade para cima (pixels/segundo)

        # Raios do buraco negro (começam pequenos)
        self.core_radius = config.BLACK_HOLE_INITIAL_CORE_RADIUS  # Núcleo central (começa pequeno)
        ...
        self.growth_rate = config.BLACK_HOLE_GROWTH_RATE  # Cresce 15 pixels de raio por segundo
    ...
    def _create_particles(self):
        ...
        for _ in range(config.BLACK_HOLE_PARTICLE_COUNT):  # Usar configuração otimizada
            ...
            particle: Particle = {
                ...
                'color': random.choice(BLACK_HOLE_PARTICLE_COLORS)  # Usar constante de cores
            }
    ...
    def update(self, dt: float):
        ...
        for particle in self.particles:
            ...
            distance_squared = dx * dx + dy * dy  # Usar distância ao quadrado (evita sqrt)
            distance = math.sqrt(distance_squared)  # Manter para cálculos que precisam do valor real
    ...
    def process_all_enemies(self, enemies: list[Any], dt: float, spawn_explosion_callback: Any = None) -> None:
