...
class PlayingScene(Scene):
    ...
    def update(self, dt: float):
        ...
        if not self.boss_fight_active and not self.pre_boss_transition and not self.level_transition_active:
        ...
        if not self.level_transition_active: self._handle_collisions()
        ...
        if self.pre_boss_transition:
            ...
            if self.warning_timer > 0:
                ...
                if self.pre_boss_timer >= 5.0: self._start_boss_fight()
        elif not self.boss_fight_active and not self.level_transition_active: self._check_level_progression()
        elif self.entity_manager.boss and self.entity_manager.boss.dead: self._end_boss_fight()

    def _all_animations_finished(self) -> bool:
        return not self.entity_manager.explosions and not self.entity_manager.bullets and not self.entity_manager.alien_bullets and not self.entity_manager.boss_lasers and not self.entity_manager.enemies

    def _handle_collisions(self):
        gain, destroyed, score_events = self.collisions.bullets_vs_enemies(self.entity_manager.bullets, self.entity_manager.enemies, self.entity_manager.explosions)
        for x, y, pts in score_events: self.entity_manager.floating_scores.append(FloatingScore(x, y, pts))
        self.score += gain
        self.total_enemies_destroyed += destroyed
        self.enemies_destroyed_in_level += destroyed

        if self.entity_manager.boss: 
            score_gain = self.collisions.bullets_vs_boss(self.entity_manager.bullets, self.entity_manager.boss, self.entity_manager.explosions, self.entity_manager.floating_scores)
            self.score += score_gain
        if self.collisions.ship_vs_enemies(self.ship, self.entity_manager.enemies, self.entity_manager.explosions): self._handle_ship_hit()
        if self.entity_manager.boss and self.collisions.ship_vs_boss(self.ship, self.entity_manager.boss, self.entity_manager.explosions): self._handle_ship_hit()
        if self.collisions.alien_bullets_vs_ship(self.ship, self.entity_manager.alien_bullets): self._handle_ship_hit()
        if self.collisions.laser_vs_ship(self.ship, self.entity_manager.boss_lasers): self._handle_ship_hit()
        
        collected_powerups = self.collisions.ship_vs_powerups(self.ship, self.entity_manager.powerups)
        if collected_powerups:
            for kind in collected_powerups:
                if kind == "life":
                    ...
                    self.ship.invuln = max(self.ship.invuln, Config.SHIELD_DURATION * 1000)
                elif kind == "double_shot":
                    self.ship.double_shot_timer = max(self.ship.double_shot_timer, Config.DOUBLE_SHOT_DURATION)
                elif kind == "speed":
                    self.ship.speed_boost_timer = max(self.ship.speed_boost_timer, Config.SPEED_BOOST_DURATION)
                elif kind == "score":
                    self.score += Config.POWERUP_SCORE_BONUS
                elif kind == "rainbow":
                    self.lives += 1
                    self.ship.lives = self.lives
                    self.ship.invuln = max(self.ship.invuln, Config.RAINBOW_DURATION * 1000)
                    self.ship.double_shot_timer = max(self.ship.double_shot_timer, Config.RAINBOW_DURATION)
                    self.ship.speed_boost_timer = max(self.ship.speed_boost_timer, Config.RAINBOW_DURATION)
                    self.score += Config.POWERUP_SCORE_BONUS * 2

    def _handle_ship_hit(self):
        if self.ship.invuln > 0 or self.game_over_sequence_active: return
        self.lives -= 1
        self.ship.lives = self.lives
        if self.lives > 0:
            ...
            self.entity_manager.explosions.append(Explosion(self.ship.rect.centerx, self.ship.rect.centery, size=100))
    ...
    def _check_level_progression(self):
        if self.enemies_destroyed_in_level >= self.level_config.enemies_to_clear:
            ...
            if not self.entity_manager.enemies:
                if self.level_config.boss_type: self.pre_boss_transition = True
                else: self._advance_to_next_level()
    ...
    def _start_boss_fight(self):
        ...
        if self.level_config.boss_type:
            self.entity_manager.boss = self.level_config.boss_type(Config.SCREEN_WIDTH / 2 - 50, 50)

    def _end_boss_fight(self):
        if not self.entity_manager.boss: return

        # Efeitos da explosão
        boss_center = (self.entity_manager.boss.x + self.entity_manager.boss.w / 2, self.entity_manager.boss.y + self.entity_manager.boss.h / 2)
        ...
        self.entity_manager.explosions.append(Explosion(boss_center[0], boss_center[1], size=120))
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_p:
                ...
                if self.shoot_cd == 0.0 and not self.level_transition_active:
                    ...
                    for pos in bullet_positions: self.entity_manager.bullets.append(Bullet(*pos))
                    self.shoot_cd = 0.15
    ...
    def render(self, surface: pygame.Surface):
        ...
        self.r.hud(self.game_surface, self.score, self.lives, self.total_enemies_destroyed, self.ship, self.level_config.level_number)

        shake_offset = (0, 0)
        if self.screen_shake_timer > 0:
            shake_offset = (random.randint(-self.screen_shake_intensity, self.screen_shake_intensity), random.randint(-self.screen_shake_intensity, self.screen_shake_intensity))
        ...
        if self.warning_timer > 0 and int(self.warning_timer * 5) % 2 == 1:
            ...
            text_rect = warning_text.get_rect(center=(Config.SCREEN_WIDTH / 2, Config.SCREEN_HEIGHT / 2))
            surface.blit(warning_text, text_rect)

        if self.game_over_sequence_active:
            progress = min(1.0, self.game_over_timer / 2.0) # Duração do fade-in principal
            ...
            title_text = self.game_over_font_title.render("GAME OVER", True, (255, 255, 255))
            title_text.set_alpha(text_alpha)
            title_rect = title_text.get_rect(center=(Config.SCREEN_WIDTH / 2, Config.SCREEN_HEIGHT / 2 - 40))
            ...
            if self.game_over_timer > restart_delay:
                ...
                score_text = self.game_over_font_subtitle.render(f"Score: {self.score}", True, (255, 255, 255))
                ...
                restart_text = self.game_over_font_subtitle.render("Pressione R para reiniciar", True, (255, 255, 255))
