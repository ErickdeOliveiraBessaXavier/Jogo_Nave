...
class SettingsView:
    ...
    def render(self, surface: pygame.Surface):
        ...
        self._draw_button(surface, self.layout_rects["back_button"], "Voltar", CUSTOM_PURPLE, alpha, offset_y)
    ...
    def _draw_card(self, surface: pygame.Surface, rect: pygame.Rect, title: str, alpha: int = 255, offset_y: int = 0):
        adjusted_rect = rect.copy()
        adjusted_rect.y += offset_y
        
        # Apenas a borda, sem fundo
        temp_surface = pygame.Surface((adjusted_rect.width + 2, adjusted_rect.height + 2), pygame.SRCALPHA)
        pygame.draw.rect(temp_surface, (*colors.GRAY, alpha), pygame.Rect(1, 1, adjusted_rect.width, adjusted_rect.height), 1, border_radius=8)
    ...
    def _draw_audio_card(self, surface: pygame.Surface, alpha: int = 255, offset_y: int = 0):
        card_rect = self.layout_rects["audio_card"].copy()
        card_rect.y += offset_y
        self._draw_card(surface, self.layout_rects["audio_card"], "Áudio", alpha, offset_y)
        ...
        for key in self.sliders:
            ...
            pygame.draw.rect(temp_bg, (10, 10, 10, alpha), temp_bg.get_rect(), border_radius=10)
            ...
            pygame.draw.rect(temp_fill, (*CUSTOM_PURPLE, alpha), fill_rect, border_radius=10)
            surface.blit(temp_fill, rect.topleft)
            
            # Borda
            temp_border = pygame.Surface((rect.width + 2, rect.height + 2), pygame.SRCALPHA)
            pygame.draw.rect(temp_border, (*colors.GRAY, alpha), pygame.Rect(1, 1, rect.width, rect.height), 1, border_radius=10)
            ...
            temp_knob = pygame.Surface((knob_rect.width + 2, knob_rect.height + 2), pygame.SRCALPHA)
            pygame.draw.rect(temp_knob, (*CUSTOM_GOLD, alpha), pygame.Rect(1, 1, knob_rect.width, knob_rect.height), border_radius=3)
    ...
    def _draw_controls_card(self, surface: pygame.Surface, alpha: int = 255, offset_y: int = 0):
        card_rect = self.layout_rects["controls_card"].copy()
        card_rect.y += offset_y
        self._draw_card(surface, self.layout_rects["controls_card"], "Instruções", alpha, offset_y)
