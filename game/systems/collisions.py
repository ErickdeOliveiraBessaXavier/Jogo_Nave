...
class Collisions:
    ...
    @staticmethod
    def get_collision_info(enemy: Meteor | Alien | ExplosiveMine | EyeEnemy | SquareMinionBoss | SlimeBoss) -> tuple[float, float, float]:
    ...
    def _destroy_enemy(
        self,
        enemy: Meteor | Alien | ExplosiveMine | EyeEnemy | SquareMinionBoss | SlimeBoss,
        enemies_list: list[Meteor | Alien | ExplosiveMine | EyeEnemy | SquareMinionBoss | SlimeBoss],
    ...
    def _apply_area_damage(
        ...
        for enemy in enemies[:]:
            ...
            if dist_sq < (damage_radius + radius) ** 2:
                ...
                elif isinstance(enemy, SlimeBoss):
                    # SlimeBoss não pode ser destruído por tiros
                    pass  # Não faz nada - imune a tiros
                else:
            ...
            for enemy in enemies[:]:
                ...
                if dist_sq < (explosion_radius + enemy_r) ** 2:
                    if isinstance(enemy, ExplosiveMine):
                        ...
                        # SlimeBoss não pode ser destruído por explosões
                        pass  # Não faz nada - imune a explosões
                    else:
                        pts, score_event = self._destroy_enemy(enemy, enemies, entity_manager)
                        score_gain += pts
    ...
    def mini_ship_bullets_vs_enemies(
        ...
        enemy_grid: SpatialGrid[Meteor | Alien | ExplosiveMine | EyeEnemy | SquareMinionBoss | SlimeBoss],
        enemies: list[
            Meteor | Alien | ExplosiveMine | EyeEnemy | SquareMinionBoss | SlimeBoss
            ...
            for enemy in potential_enemies:
                ...
                if b_rect.colliderect(enemy_rect):  # Usa cache
                    ...
                    if isinstance(enemy, ExplosiveMine):
                        ...
                        # SlimeBoss não pode ser destruído por tiros, mas a bala sim
                        # Criar explosão visual no ponto de impacto
                        entity_manager.spawn_explosion(b.x, b.y, size=20)
                        # Som de impacto (mesmo som de dano ao boss)
                        sound_manager.play_boss_damage()
                        ...
                        for nearby_enemy in area_query:
                            ...
                            if dist_sq < explosion_radius**2:
                                if isinstance(nearby_enemy, ExplosiveMine):
                                    ...
                                    pass  # Não faz nada - imune a explosões
                                elif isinstance(nearby_enemy, SlimeBoss):
                                    # SlimeBoss não pode ser destruído por explosões de área
                                    pass  # Não faz nada - imune a explosões
    ...
    def ship_vs_enemies(
        ...
        for enemy in potential_enemies:
            ...
            if enemy and ship.rect.colliderect(enemy_rect):
                ...
                if isinstance(enemy, Meteor):
                    sound_manager.play_explosion_asteroid()
                elif isinstance(enemy, (Alien, SquareMinionBoss, SlimeBoss)):
                    sound_manager.play_explosion_alien()
                else:
            ...
            for enemy in enemies[:]:
                ...
                if enemy_rect.clipline(line):
                    ...
                    if isinstance(enemy, Meteor):
                        ...
                        # SlimeBoss não pode ser destruído por lasers
                        pass  # Não faz nada - imune a lasers
