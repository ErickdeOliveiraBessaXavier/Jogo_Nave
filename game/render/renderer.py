...
class Renderer:
    ...
    def overlay(self, surface: pygame.Surface, title: str, subtitle: str = ""):
        ...
        rect = t.get_rect(center=(Config.SCREEN_WIDTH // 2, Config.SCREEN_HEIGHT // 2 - 40))
        surface.blit(t, rect)
        if subtitle:
            s = self.font_medium.render(subtitle, True, colors.WHITE)
            rect = s.get_rect(center=(Config.SCREEN_WIDTH // 2, Config.SCREEN_HEIGHT // 2 + 20))
            surface.blit(s, rect)
    ...
    def effects_on_ship(self, surface: pygame.Surface, ship: "Ship"):
        ...
        if invuln > 0:
            ...
            pygame.draw.circle(halo, (0, 120, 255, 120), (radius + 3, radius + 3), radius, width=3)
