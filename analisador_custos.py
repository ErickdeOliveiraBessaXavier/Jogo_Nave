...
def analisar_profiling(prof_file: str) -> dict[str, Any]:
    ...
    custos_por_componente: dict[str, float] = {
        "Renderização": 0,
        "Sistema de Som": 0,
        ...
        if "filename:lineno(function)" in line:
            ...
            try:
                ...
                if len(parts) >= 6:
                    ...
                    if any(
                        x in function_info
                        for x in ["renderer.py", "blit", "fill", "draw"]
                    ):
                        custos_por_componente["Renderização"] += cumtime
                    elif any(
                        x in function_info.lower() for x in ["sound", "music", "audio"]
                    ):
                        custos_por_componente["Sistema de Som"] += cumtime
                    elif any(
                        x in function_info
                        for x in ["entity_manager", "meteor.py", "bullet", "explosion"]
                    ):
                        custos_por_componente["Gerenciamento de Entidades"] += cumtime
                    elif any(
                        x in function_info.lower() for x in ["collision", "collisions"]
                    ):
                        custos_por_componente["Colisões"] += cumtime
                    elif any(
                        x in function_info.lower()
                        for x in ["explosion", "particle", "effect"]
                    ):
                        custos_por_componente["Sistema de Partículas"] += cumtime
                    elif any(
                        x in function_info.lower()
                        for x in ["hud", "font", "text", "ui"]
                    ):
                        custos_por_componente["Interface/HUD"] += cumtime
                    elif any(
                        x in function_info
                        for x in ["pygame.display.flip", "display.flip"]
                    ):
                        custos_por_componente["Display/Sync"] = (
                            custos_por_componente.get("Display/Sync", 0) + cumtime
                        )
                    elif any(x in function_info for x in ["threading", "thread"]):
                        custos_por_componente["Threading"] = (
                            custos_por_componente.get("Threading", 0) + cumtime
                        )
                    elif any(x in function_info for x in ["time.sleep", "sleep"]):
                        custos_por_componente["Sleep/Timing"] = (
                            custos_por_componente.get("Sleep/Timing", 0) + cumtime
                        )
                    else:
...
def gerar_relatorio_custos(analise: dict[str, Any]) -> None:
    ...
    custos = cast(dict[str, float], analise["custos_por_componente"])
    sorted_custos: list[tuple[str, float]] = sorted(
        custos.items(), key=lambda x: x[1], reverse=True
    )
    ...
    for componente, custo in sorted_custos:
        if custo > 0:
            ...
            print(
                f"{componente:<25} {custo:>6.3f}s ({porcentagem:>5.1f}%) - Impacto: {fps_impact:>6.1f} FPS"
            )
    print()
...
def criar_grafico_custos(
    analise: dict[str, Any], output_file: str = "custos_componentes.png"
) -> None:
    ...
    try:
        ...
        _, texts, _ = ax.pie(
            sizes, labels=labels, autopct="%1.1f%%", startangle=90, shadow=True
        )

        ax.axis("equal")
        plt.title(
            "Distribuição de Custos por Componente\n" + ".1f"
            f'FPS Médio: {analise["fps"]:.1f}'
        )
...
def main():
    ...
    parser.add_argument(
        "--chart", action="store_true", help="Gerar gráfico visual dos custos"
    )
